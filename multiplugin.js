(function () {
    ...
    Lampa.Lang.add({
        ...
        mp_category_plugins: {
            ru: 'Категории плагинов',
            uk: 'Категорії плагінів',
            en: 'Plugin Categories'
        },
        mp_updated: {
            ru: 'Обновлено: ',
            uk: 'Оновлено: ',
        ...
        mp_load_online_only: {
            ru: 'Загрузить только онлайн',
            uk: 'Завантажити тільки онлайн',
            en: 'Load Online Only'
        },
        mp_management: {
            ru: 'Управление',
        ...
        mp_export_to_lampa: {
            ru: 'Экспорт плагинов в Lampa',
            uk: 'Експорт плагінів в Lampa',
            en: 'Export Plugins to Lampa'
        },
        mp_disable_all: {
            ru: 'Выключить все плагины',
            uk: 'Вимкнути всі плагіни',
            en: 'Disable All Plugins'
        },
        mp_reload_lampa: {
        ...
        mp_no_plugins: {
            ru: 'Включённых плагинов нет',
            uk: 'Увімкнених плагінів немає',
            en: 'No enabled plugins'
        },
        mp_no_plugins_category: {
        ...
        mp_all_disabled: {
            ru: 'Все плагины отключены',
            uk: 'Всі плагіни вимкнено',
            en: 'All plugins disabled'
        },
        mp_sync_complete: {
        ...
        mp_online_enabled: {
            ru: 'Онлайн-плагины включены',
            uk: 'Онлайн-плагіни увімкнено',
            en: 'Online plugins enabled'
        },
        mp_export_complete: {
            ru: 'Экспорт завершён',
        ...
        mp_confirm_online: {
            ru: 'Загрузить и включить только плагины категории "Онлайн"?',
            uk: 'Завантажити та увімкнути тільки плагіни категорії "Онлайн"?',
            en: 'Load and enable only "Online" category plugins?'
        },
        mp_confirm_disable_all: {
            ru: 'Вы уверены? Все плагины будут отключены',
            uk: 'Ви впевнені? Усі плагіни будуть вимкнено',
            en: 'Are you sure? All plugins will be disabled'
        },
        mp_export_message: {
    ...
    // ────────────────────────────────────────────────
    // Хелпер для переводов из plugins_list.js
    // ────────────────────────────────────────────────
    function tr(value) {
        if (!value) return '';
    ...
    function showInfo() {
        ...
        if (info.added.length) {
            ...
            info.added.forEach(p => {
                ...
                html += '• <b>' + name + '</b><br>';
                if (p.description) {
                    html += '<div style="color:#bfbfbf; font-size:0.9em; margin-left:18px;">' + tr(p.description) + '</div>';
                }
                html += '<br>';
            });
            html += '<br>';
            ...
            buttons: [
                ...
                },
                {
                    ...
                    onSelect: function () {
                        ...
                        Lampa.Noty.show(Lampa.Lang.translate('mp_all_disabled'));
                        Lampa.Modal.close();
                        Lampa.Controller.toggle(prev);
                    }
                }
        ...
        fetch(syncUrl, { cache: 'no-cache' })
            ...
            .then(text => {
                ...
                if (listMatch && listMatch[1]) {
                    ...
                    const enabledSet = new Set(Lampa.Storage.get(ENABLED_KEY, []));
                    newList
                        .filter(p => tr(p.category) === 'Онлайн' || tr(p.category) === 'Online')
